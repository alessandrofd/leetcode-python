...
class Solution:
    def countPaths(self, grid: List[List[int]]) -> int:
        ...
        def visit(row: int, col: int, prev: int, memo: List[List[int]]) -> int:
            ...
            for d_row, d_col in deltas:
                paths += visit(row + d_row, col + d_col, grid[row][col], memo)

            memo[row][col] = paths
            return memo[row][col]

        return (
            sum(visit(row, col, 0, memo) for row, col in product(range(m), range(n)))
            % 1_000_000_007
        )
