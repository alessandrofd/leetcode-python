...
class Solution:
    def minimumTime(self, n: int, relations: list[list[int]], time: list[int]) -> int:
        ...
        courses_by_prereqs = defaultdict(list)
        count_prereqs = [0] * (n + 1)
        max_time = [0] * (n + 1)

        for prereq, course in relations:
            courses_by_prereqs[prereq].append(course)
            count_prereqs[course] += 1

        queue = deque()
        for course in range(1, n + 1):
            if count_prereqs[course] == 0:
                queue.append(course)
                max_time[course] = time[course - 1]

        while queue:
            prereq = queue.popleft()
            for course in courses_by_prereqs[prereq]:
                max_time[course] = max(
                    max_time[course], max_time[prereq] + time[course - 1]
                )
                count_prereqs[course] -= 1
                if count_prereqs[course] == 0:
                    queue.append(course)
