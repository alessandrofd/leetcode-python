...
class Solution:
    ...
    def minScore(self, num_cities: int, roads: List[List[int]]) -> int:
        ...
        parents = list(range(num_cities + 1))
        ...
        scores = [10**4] * (num_cities + 1)

        for origin, dest, score in roads:
            origin, dest = find(origin), find(dest)
            scores[dest] = min(scores[dest], scores[origin], score)
            parents[origin] = dest
