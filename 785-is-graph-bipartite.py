...
class Solution:
        ...
        for node in range(n):
            ...
            while stack:
                ...
                for neighbor in graph[top]:
                    if members[neighbor] == members[top]:
                        return False
                
                    if members[neighbor] is None:
                        members[neighbor] = not members[top]
    ...
    def isBipartite_recurse(self, graph: List[List[int]]) -> bool:
        ...
        def recurse(nodes, group):
            for node in nodes:
                if members[node] is not None:
                    if members[node] != group: 
                        return False
                else:                    
                    members[node] = group
                    if not recurse(graph[node], not group):
                        return False
        ...
        for node in range(n):
            if members[node] is None: 
                members[node] = True
                if not recurse(graph[node], False):
                    return False
