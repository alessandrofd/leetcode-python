...
class Solution:
    def new21Game(self, max_hand: int, stop_hand: int, max_card: int) -> float:
        if stop_hand == 0:
            return 1.0
        if max_hand >= stop_hand + max_card:
            return 1.0

        probs = [0.0] * (max_hand + 1)
        probs[0] = 1.0
        sum_probs = probs[0]

        for hand in range(1, max_hand + 1):
            probs[hand] = sum_probs / max_card
            if hand < stop_hand:
                sum_probs += probs[hand]
            if hand >= max_card:
                sum_probs -= probs[hand - max_card]

        return sum(probs[stop_hand : max_hand + 1])
