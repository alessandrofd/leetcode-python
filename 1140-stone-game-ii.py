...
class Solution:
    ...
    def stoneGameII_recursion(self, piles: List[int]) -> int:
        ...
        n = len(piles)
        memo = [[[-1] * (n + 1) for i in range(n + 1)] for i in range(2)]

        def f(player, i, m):
            if i == n:
                return 0
            if memo[player][i][m] >= 0:
                return memo[player][i][m]

            result = 0 if player == 0 else int(1e6 + 1)
            stones = 0
            for x in range(1, min(2 * m, n - i) + 1):
                stones += piles[i + x - 1]
                if player == 0:
                    result = max(result, stones + f(1, i + x, max(m, x)))
                else:
                    result = min(result, f(0, i + x, max(m, x)))
    ...
    def stoneGameII_recursion_cache(self, piles: List[int]) -> int:
        """solution method"""
    ...
    def stoneGameII_dp_cache(self, piles: List[int]) -> int:
        """solution method"""
...
def test_solution():
    ...
    funcs = [
        ...
        Solution().stoneGameII_recursion_cache,
        Solution().stoneGameII_dp,
        Solution().stoneGameII_dp_cache,
    ]
