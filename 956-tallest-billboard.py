    ...
    def tallestBillboard(self, rods: List[int]) -> int:
        ...
        for rod in rods:
            ...
            for diff, tallest in dp.items():
                # Vara adicionada ao lado mais baixo
                new_dp[diff + rod] = max(new_dp[diff + rod], tallest + rod)

                # Vara adicionada ao lado mais alto
                tallest += 0 if diff > rod else rod - diff
                diff = abs(diff - rod)
                new_dp[diff] = max(new_dp[diff], tallest)
            dp = new_dp
        return dp[0]
