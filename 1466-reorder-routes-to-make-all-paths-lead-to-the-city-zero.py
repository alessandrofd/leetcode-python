...
class Solution:
    ...
    def minReorder(self, num_cities: int, connections: List[List[int]]) -> int:
        ...
        tree = [[] for _ in range(num_cities)]
        for u, v in connections:
            tree[u].append((v, False))
            tree[v].append((u, True))

        reoriented_roads = 0

        def dfs(node, parent):
            nonlocal reoriented_roads
            for next_node, oriented in tree[node]:
                if next_node != parent:
                    if not oriented:
                        reoriented_roads += 1
                    dfs(next_node, node)
