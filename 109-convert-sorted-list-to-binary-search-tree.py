...
class Solution:
    ...
    def sortedListToBST_array(self, head: Optional[ListNode]) -> Optional[TreeNode]:
        ...
        node = head
        while node:
            indexed_nodes.append(node.val)
            node = node.next

        def recurse(start, end):
            """Constrói recursivamente os ramos da árvore"""
            ...
            mid = (start + end) // 2
            node = TreeNode(
                indexed_nodes[mid], recurse(start, mid - 1), recurse(mid + 1, end)
            )

            return node

        return recurse(0, len(indexed_nodes) - 1)
    ...
    def sortedListToBST_no_array(self, head: Optional[ListNode]) -> Optional[TreeNode]:
        ...
        def recurse(start, end):
            """Constói recursivamente os ramos da árvore"""
