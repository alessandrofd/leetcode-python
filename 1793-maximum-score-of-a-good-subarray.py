...
class Solution:
    def maximumScore_bin_search(self, nums: list[int], k: int) -> int:
        ...
        def solve(nums: list[int], k: int) -> int:
            ...
            max_score = 0
            for i, curr_min in enumerate(right):
                left_len = len(left) - bisect_left(left, curr_min)
                right_len = i + 1
                total_len = left_len + right_len
                max_score = max(max_score, curr_min * total_len)
    ...
    def maximumScore_monotonic_stack(self, nums: list[int], k: int) -> int:
        ...
        for i, curr_min in enumerate(nums):
            if left[i] < k < right[i]:
                max_score = max(max_score, curr_min * (right[i] - left[i] - 1))
    ...
    def maximumScore_greedy(self, nums: list[int], k: int) -> int:
        ...
        while left > 0 or right < n - 1:
            left_num = 0 if left == 0 else nums[left - 1]
            right_num = 0 if right == n - 1 else nums[right + 1]
