...
class Solution:
    def largestPathValue(self, colors, edges):
        ...
        parent_to_child = [[] for _ in range(num_nodes)]
        incoming_edges = [0 for _ in range(num_nodes)]
        for parent, child in edges:
            parent_to_child[parent].append(child)
            incoming_edges[child] += 1

        max_colors = [[0] * 27 for _ in range(num_nodes)]

        queue = [node for node in range(num_nodes) if incoming_edges[node] == 0]
        ...
        while queue:
            ...
            max_colors[node][node_color] += 1
            result = max(result, max_colors[node][node_color])
            ...
            for child in parent_to_child[node]:
                ...
                for color in range(27):
                    max_colors[child][color] = max(
                        max_colors[child][color], max_colors[node][color]
                    )

        return result if num_visited_nodes == num_nodes else -1
