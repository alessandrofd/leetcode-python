...
class Solution:
    ...
    def strangePrinter_top_down_dp(self, s: str) -> int:
        ...
        # Elimina letras repetidas sequanciais
        s = "".join([c for i, c in enumerate(s) if i == 0 or c != s[i - 1]])
        n = len(s)

        # A lista next_occurance traz a próxima ocorrência da letra na string s.
        # Em sua última ocorrência, os valores contidos no dicionário
        # last_occurance, nós atualizamos a lista para um índice além dos
        # limites da string, neste caso o próprio comprimento da string, para
        # facilitar comparação mais a frente.
        last_occurance = {}
        next_occurance = list(range(n))
        for i, c in enumerate(s):
            if c in last_occurance:
                next_occurance[last_occurance[c]] = i
            last_occurance[c] = i

        for i in last_occurance.values():
            next_occurance[i] = n
        ...
        def solve(left, right):
            ...
            # Garante que no processamento subsequente left != right
            if s[left] == s[right]:
                return solve(left + 1, right)

            result = 1 + solve(left + 1, right)

            i = next_occurance[left]
            while i < right:
                result = min(result, 1 + solve(left + 1, i) + solve(i + 1, right))
                i = next_occurance[i]
