...
class Solution:
    def numMusicPlaylists(
        ...
        *    => dp[i][j] = dp[i-1][j] * (j - numLeadSongs)
        ...
        MOD = 10**9 + 7
        dp = [[0] * (num_songs + 1) for _ in range(length_playlist + 1)]
        dp[0][0] = 1

        for i in range(1, length_playlist + 1):
            for j in range(1, min(i, num_songs) + 1):
                dp[i][j] = (dp[i][j] + dp[i - 1][j - 1] * (num_songs - (j - 1))) % MOD
                if j > num_lead_songs:
                    dp[i][j] = (dp[i][j] + dp[i - 1][j] * (j - num_lead_songs)) % MOD

        return dp[length_playlist][num_songs]
